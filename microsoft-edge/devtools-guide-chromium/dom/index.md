...
1.  [ **属性の変更を中断**] で、[ **Sauerkraut** ] を右クリックし、[ **検査**] を選びます。  
    ...
    1.  次の手順では、ページのコードを一時停止するボタンをクリックするように指示されます。  ページが一時停止されると、ページをスクロールすることはできなくなります。  **Resume Script** ![ ][ImageResumeScriptIcon] ページを再びスクロールできるようにするには、[スクリプトの再開] をクリックします。
    ...
    1.  **Resume Script** ![ ][ImageResumeScriptIcon] 前に説明したように、[スクリプトの再開] をクリックします (スクリプトを再開します)。  
...
1.  [ **ノード削除時の中断**] で、[ **Neuromancer** ] を右クリックし、[ **検査**] を選びます。  
    ...
    1.  [ **スクリプトの再開** ] ![ をクリックします (スクリプトを再開し ][ImageResumeScriptIcon] ます)。  
