...
このページでは、Microsoft Edge DevTools コンソールに関連する機能のリファレンスを示します。  ログに記録されたメッセージを表示して JavaScript を実行するには、既にコンソールを使用していることを前提としています。  表示されない場合は、本体の [ [JavaScript の実行を開始][DevToolsConsoleJavascript] する] に移動して、 [コンソールでメッセージの記録を開始][DevToolsConsoleLog]します。  

API 参照を検索する場合は、 `console.log()` 「 [コンソール API リファレンス][DevToolsConsoleApi]」をご覧ください。  次のような関数のリファレンスについては `monitorEvents()` 、「 [コンソールユーティリティ API リファレンス][DevToolsConsoleUtilities]」を参照してください。  
...
[ `Escape` **カスタマイズと制御** ] を選択または選択して ( `...` \)、[ **コンソールドローワの表示**] を選択します。  
...
[ **コンソールの設定** ![ ] (コンソールの設定アイコン ][ImageSettingsButtonIcon] \) を選びます。  
...
[ **コンソールサイドバーを表示** ] を選択 ![ すると ][ImageShowConsoleSidebarIcon] 、サイドバーが表示され、フィルター処理に役立ちます。  
...
[コンソールのサイドバーを開き](#open-the-console-sidebar) 、[ **ユーザーメッセージ** ] を選択して、ページの JavaScript から送信されたメッセージのみを表示します。  
...
キーを押して、 `Up Arrow` コンソールで前に実行した JavaScript 式の履歴を順に切り替えます。  この `Enter` 式をもう一度実行する場合に選択します。  

### ライブ式を使用して、式の値をリアルタイムで見る  

コンソールで同じ JavaScript 式を繰り返し入力したことがわかった場合は、 **ライブ式**の作成が簡単になることがあります。  **ライブ式**で式を1回入力し、本体の先頭に固定します。  この式の値は、ほぼリアルタイムで更新されます。  「 [ライブ式で Real-Time の JavaScript の式の値を見る][DevToolsConsoleLiveExpressions]」をご覧ください。  
...
*   [ **コンソールのクリア** ] ( ![ コンソールのクリア) を選択し ][ImageClearConsoleIcon] ます。  
*   メッセージを右クリックし、[ **コンソールのクリア**] を選択します。  
*   コンソールに入力して、 `clear()` を選択し `Enter` ます。  
*   `console.clear()`Web ページの JavaScript から実行します。  
*   `Control` + `L` コンソールがフォーカスされているときにを選択します。  
