...
DevTools は、JavaScript ファイルを生成したコンパイラにマップファイルの名前のコメントが含まれているかどうかを確認します。 たとえば、コンパイラによって*myfile.txt*が*myfile.txt に圧縮*された場合、マップファイル (myfile.txt) が生成される場合もあり*ます。* 次のように、圧縮されたファイルにコメントを追加します。
