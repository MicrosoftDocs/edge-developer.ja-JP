...
| debuggerAddress | 文字列 |  | 接続先のデバッガー サーバーのアドレス (例: `hostname/ip:port` . `127.0.0.1:38947` |
| detach | ブール値 | `false` | 場合は、WebDriver サービスが終了すると、WebDriver ローカルエンドがセッションを閉じていなくても `false` 、Microsoft Edge が終了します。  場合は、WebDriver ローカルエンドがセッションを閉じる場合にのみ Microsoft `true` Edge が終了します。  WebDriver ローカル エンドがセッションを閉じない場合、Microsoft Edge インスタンスによって使用される一時ユーザー データ フォルダー `true` `EdgeDriver` はクリーンアップされません。 |  
...
| localState | ディクショナリ |  | ユーザー設定の名前と値で構成される各エントリを持つ辞書。  ユーザー設定は、ユーザー データ フォルダー内のローカル状態ファイルに適用されます。 |  
...
| prefs | ディクショナリ |  | ユーザー設定の名前と値で構成される各エントリを持つ辞書。  ユーザー設定は、使用中のユーザー プロファイルにのみ適用されます。  たとえば、Microsoft Edge の `Preferences` ユーザー データ フォルダー内のファイルに移動します。 |  
...
| wdpUsername | 文字列 |  | Windows Device Portal サーバーに接続するときに使用するオプションのユーザー名。  サーバーで認証が有効になっている場合は必須です。 |  
...
| msedge.userDataDir | 文字列 | Microsoft Edge インスタンスによって使用されるユーザー データ フォルダーへのパス。 |  
