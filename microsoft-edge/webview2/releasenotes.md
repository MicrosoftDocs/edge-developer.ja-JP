...
> [!NOTE]
> NuGet パッケージを更新した後、アプリを再コンパイルします。  

## 1.0.721 プレリリース  

リリース日: 2020 年 12 月 8 日  

[NuGet パッケージ][NuGetGallery1.0.721-prerelease] \|Microsoft Edge バージョン 86.0.616.0 以上。  

#### 全般的な情報  

> [!IMPORTANT]
>  **Breaking Change**: WebView2 pre-release packages 1.0.707 and 0.9.628 been deprecated. これらのパッケージを使用して開発を中止してください。  
...
*   WebView2 内での [ドラッグ アンド ドロップ&サポート][ReferenceWin32Icorewebview2experimentalcompositioncontroller3] が追加されました。  
*   DPI サポートを処理する API が追加されました。  
    ...
    *   [BoundsMode][ReferenceWin32Icorewebview2experimentalcontrollerViewWebview210721PrereleaseGetBoundsMode]プロパティが追加され、境界がロジック ピクセルであり、WebView が for WebView2 ピクセル表示を使用し、WebView が物理サイズを取得するために WebView に適用されるのを許可するプロパティが追加されました。 `RasterizationScale` `RasterizationScale` `Bounds`
...
[NuGet パッケージ][NuGetGallery1.0.664.37] \|最小 WebView2 ランタイム バージョン 86.0.616.0。  

#### 全般的な情報  

> [!IMPORTANT]
> **Announcement**: .NET WPF/WinForms WebView2 is now Generally Available \(GA\).  このリリースから、リリース SDK は前方互換性があります。  詳細については、GA のお知らせ [ブログの投稿に移動します][MicrosoftDevblogDotnetAnnouncingGeneralAvailabilityForMicrosoftEdgeWebview2ForNetFixedDistributionMethod]。  
...
*   `CoreWebView2NewWindowRequestedEventArgs.Handled` 新しいウィンドウが \([\#549 \)][GithubMicrosoftedgeWebviewfeedbackIssue549]および \([\#560][GithubMicrosoftedgeWebviewfeedbackIssue560]\) で開かれません。  
...
[NuGet パッケージ][NuGetGallery1.0.674-prerelease] \|最小 WebView2 ランタイム バージョン 86.0.616.0。  

#### 全般的な情報  

*   ナビゲーション中にポスト データまたは追加の要求ヘッダーを提供できる [NavigateWithWebResourceRequest][ReferenceWin32Icorewebview2experimentalNavigatewithwebresourcerequest10674] メソッドが追加されました。
...
*   開発者が [WebView2][ReferenceWin32Icorewebview2experimentalGetCookiemanager10674] セッションを認証したり、WebView から Cookie を取得して他のツールを認証したりできる Cookie 管理 API が追加されました。  言語/フレームワーク固有の改善を行う予定です。  詳細については、「API レビュー [: Cookie の管理」に移動します][GithubMicrosoftedgeWebview2AnnouncementIssue2]。
*   [WebResourceResponseReceived][ReferenceWin32Icorewebview2experimentalAddWebresourceresponsereceived10674]イベントを更新し、不変[の WebResourceResponseView][ReferenceWin32Icorewebview2experimentalwebresourceresponseview10674]と[WebResourceResponseReceivedEventArgs::P opulateResponseContent][ReferenceWin32Icorewebview2experimentalwebresourceresponsereceivedeventargsPopulateresponsecontent09628]を[WebResourceResponseView::GetContent][ReferenceWin32Icorewebview2experimentalwebresourceresponseviewGetcontent10674]に追加しました。
...
*   WebView2 静的ライブラリを使用する場合、開発者は version.lib を含める必要がなくなりました。

#### .NET  

*   [CoreWebView2Environment][DotnetApiMicrosoftWebWebview2CoreCorewebview2]変数を公開するために CoreWebView2 クラスを更新しました。  
...
*   .NET [WebResourceRequested][DotnetApiMicrosoftWebWebview2CoreCorewebview2Webresourcerequested] API を追加しました。  \([\#219][GithubMicrosoftedgeWebviewfeedbackIssue219]\)。  
...
[NuGet パッケージ][NuGetGallery1.0.622.22] \|最小 WebView2 ランタイム バージョン 86.0.616.0。  
...
*  [Evergreen WebView2 ランタイムとインストーラーは][Webview2ConceptsDistributionUnderstandRuntimeInstaller] GA です。  Bootstrapper、Bootstrapper のダウンリンク リンク、および Evergreen ランタイムのスタンドアロン インストーラーは [、Microsoft Edge WebView2 で利用できます][MicrosoftDeveloperMicrosoftEdgeWebView2]。  インストール ワークフローのサンプル コードは [、WebView2Samples レポでも使用できます][GithubMicrosoftedgeWebview2samplesMain]。  
...
[NuGet パッケージ][NuGetGallery0.9.622.11] \|最小 WebView2 ランタイム バージョン 86.0.616.0。  
...
*   `ICoreWebView2NewWindowRequestedEventArgs` [WindowFeatures プロパティと][ReferenceWin32Icorewebview2newwindowrequestedeventargsGetWindowfeatures09622]、関連する[ICoreWebView2WindowFeatures][ReferenceWin32Icorewebview2windowfeatures09622]を含む更新を行いました。  \([\#293][GithubMicrosoftedgeWebviewfeedbackIssue293]\)。  
...
*   [AdditionalBrowserArguments][ReferenceWin32Icorewebview2environmentoptionsPutAdditionalbrowserarguments09622] set using `ICoreWebView2EnvironmentOptions` aren't overridden with environment variables or registry values.  詳細については [、CreateCoreWebView2EnvironmentWithOptions に移動します][ReferenceWin32IdlCreatecorewebview2environmentwithoptions09622]。  

## 0.9.579  

リリース日: 2020 年 7 月 20 日  

[NuGet パッケージ][NuGetGallery0.9.579] \|Microsoft Edge バージョン 86.0.579.0 以上。  

#### 全般的な情報  

*   > [!IMPORTANT]
    > **Announcement**: Evergreen WebView2 Runtime and installer is released for preview.  詳細については、[Distribution of WebView2][Webview2ConceptsDistributionUnderstandRuntimeInstallerPreview] に移動します。  
*   > [!IMPORTANT]
...
*   WebView で無効にされたポップアップ ブロック。  詳細については、イベントの [IsUserInitiated][ReferenceWin32Icorewebview2newwindowrequestedeventargsGetIsuserinitiated09538] プロパティに移動 `NewWindowRequested` します。  
*   Ensured WebView navigation starting event is fired for `about:blank` .  現在では `NavigationStarting` 、すべてのナビゲーションに対してイベントが発生しますが、iframe srcdoc または iframe srcdoc の取り消しはサポートされていないので `about:blank` 無視されます。  
*   `edge:// URI`WebView でブロックされるスキーム。  
...
*   WebResource 要求に対する応答を WebView が受信して処理した後に発生する試験的な [WebResourceResponseReceived][ReferenceWin32Icorewebview2experimentalAddWebresourceresponsereceived09538] イベントが追加されました。  認証ヘッダーがある場合は、応答オブジェクトに含まれます。  
...
[NuGet パッケージ][NuGetGallery0.9.538] \|Microsoft Edge バージョン 85.0.538.0 以上。  
...
*   元のホスト オブジェクト シリアライザー マーカーがプロキシ オブジェクトに設定され、JavaScript コールバック \([#148][GithubMicrosoftedgeWebviewfeedbackIssue148]\) でパラメーターとして渡された場合にホスト オブジェクトとしてシリアル化戻す[AddHostObjectToScript][ReferenceWin32Icorewebview2Addhostobjecttoscript09538]が更新されました。  
...
*   視覚的なホスティングとウィンドウ機能の試験的な API の [サポートが追加されました][ConceptsVersioningExperimentalApis]。  
*   > [!IMPORTANT]
...
[NuGet パッケージ][NuGetGallery0.9.515-prerelease] \|Microsoft Edge バージョン 84.0.515.0 以上。  

*   > [!IMPORTANT]
    > **お**知らせ : WebView2 では、プレリリース パッケージの .NET Framework 4.6.2 以降および .NET Core 3.0 以降で Windows フォームと WPF **が**サポートされます。  
*   WPF アプリケーションの作成の詳細については [、「WPF][GettingstartedWpf] の概要ガイド」および WPF 固有の API 用 WebView2 [WPF リファレンス][DotnetApiMicrosoftWebWebview2Wpf] に移動してください。  
*   Windows フォーム アプリケーションの構築の詳細については [、「Windows フォーム][GettingstartedWinforms] の概要ガイド」および「Windows フォーム固有 API 用 WebView2 [Windows][DotnetApiMicrosoftWebWebview2Winforms] フォーム リファレンス」を参照してください。  
...
[NuGet パッケージ][NuGetGallery0.9.488] \|Microsoft Edge バージョン 84.0.488.0 以上。  
...
*   [FrameNavigationCompleted イベントが追加][ReferenceWin32Icorewebview2AddFramenavigationcompleted09488]されました。  これで、iframe がナビゲーションを完了すると、イベントが発生し、ナビゲーションの成功とナビゲーション ID が返されます。  
*   [ICoreWebView2EnvironmentOptions][ReferenceWin32Icorewebview2environmentoptions09488]インターフェイスが追加されました。このインターフェイスは、アプリケーションターゲットの WebView2 ランタイムのバージョンを判断するために使用できます。  
*   [IsBuiltInErrorPageEnabled 設定を追加][ReferenceWin32Icorewebview2settingsGetIsbuiltinerrorpageenabled09488]しました。  これで、ナビゲーションエラーとレンダリング プロセスの失敗に関する組み込みのエラー ページを有効または無効にできます。  
...
*   ビジュアル ホスティング API にアクセスできる、最初の独立した WebView2 プレリリース パッケージをリリースしました。  WebView チームは、 [新しい試験的な API を含む APISample][GithubMicrosoftedgeWebview2samplesMain] を更新しました。  
    ...
    *   [ICoreWebView2ExperimentalCursorChangedEventHandler][ReferenceWin32Icorewebview2experimentalcursorchangedeventhandler09488]が追加されました。このイベントは、WebView 上のマウス カーソルを変更する必要がある場合にアプリに指示します。  WebView のテキスト ボックスの上にマウスポインターを置く場合、カーソルは矢印からセレクターに変わります。  アプリ `cursor` のプロパティは、WebView のマウス カーソルの `CompositionController` 現在の位置を示します。  

## 0.9.430  

[NuGet パッケージ][NuGetGallery0.9.430] \|Microsoft Edge バージョン 82.0.430.0 以上。  

WebView2 SDK は正式な Win32 C++ ベータ版で、フィードバックからのいくつかの機能要求が組み込まれています。  WebView チームは、変更が大きかったリリースの数を制限しようとする。  一般提供に近づくと、いくつかの重要な変更がベータリリースに組み込まれる予定です。  
...
*   > [!IMPORTANT]
    > **Breaking Change**: Split [DocumentStateChanged][ReferenceWin32Iwebview2webviewAddDocumentstatechanged08190] into three components:  [SourceChanged,][ReferenceWin32Icorewebview2AddSourcechanged09430] [ContentLoading][ReferenceWin32Icorewebview2AddContentloading09430], and [HistoryChanged][ReferenceWin32Icorewebview2AddHistorychanged09430].  ソース URL が変更された場合、 `SourceChanged` イベントが発生します。  履歴の状態が変更された場合、 `HistoryChanged` イベントが発生します。  この `ContentLoading` イベントは、新しいドキュメントが読み込まれるときに、最初のスクリプトの前に発生します。  
...
*   [NotifyParentWindowPositionChanged が][ReferenceWin32Icorewebview2hostNotifyparentwindowpositionchanged09430]追加されました。これにより、ステータス バーはウィンドウ モードでウィンドウをフォローできます。  また、アクセシビリティ機能を機能するために、この変更をウィンドウレス モードで実装します。  
*   リモート オブジェクトからページにアクセスできるかどうかをグローバルに制御する [AreRemoteObjectsAllowed][ReferenceWin32Icorewebview2settingsGetAreremoteobjectsallowed09430] 設定が追加されました。  既定では有効 `AreRemoteObjectsAllowed` になっています。つまり [、AddRemoteObject][ReferenceWin32Icorewebview2Addremoteobject09430] によって追加されたリモート オブジェクトはページからアクセスできます。  無効 `AreRemoteObjectsAllowed` にした場合、オブジェクトはページからアクセスできません。  変更は、次のナビゲーション イベントに適用されます。  
*   [IsZoomControlEnabled][ReferenceWin32Icorewebview2settingsGetIszoomcontrolenabled09430]設定が追加され、ユーザーが \(または + マウス ホイール\) を使用して WebView のズームに影響を与えるのを `ctrl` + `+` `ctrl` + `-` `ctrl` 防ぐ機能が追加されました。  設定を無効にした場合でも [、put_ZoomFactorを][ReferenceWin32Icorewebview2hostPutZoomfactor09430] 使ってズームを設定できます。  
*   ZoomFactor が現在の WebView にのみ適用されるに変更されました。  現在の WebView に対するズームの変更は、同じ原点のサイトに移動した他の WebView には影響を与えいません。  詳細については、次のリンク[get_ZoomFactor。][ReferenceWin32Icorewebview2hostGetZoomfactor09430]  
...
*   JavaScript ダイアログ イベントのダイアログの種類のサポートが追加され、列挙 `beforeunload` [CORE_WEBVIEW2_SCRIPT_DIALOG_KIND_BEFOREUNLOAD][ReferenceWin32Icorewebview2CoreWebview2ScriptDialogKind09430] が追加されました。  
...
[NuGet パッケージ][NuGetGallery0.8.355] \|Microsoft Edge バージョン 80.0.355.0 以上。

*   リリースされた WebView2API サンプルは、WebView2 SDK の包括的なガイドです。  詳細については [、APISample に移動します][GithubMicrosoftedgeWebview2samplesApisample]。  
...
*   WebView2 内で実行されているスクリプトがアプリケーションの Win32 コンポーネントから IDispatch オブジェクトにアクセスし、IDispatch オブジェクトのプロパティにアクセスできる機能が追加されました `Native Object Injection` 。  詳細については [、AddRemoteObject][ReferenceWin32Iwebview2webview4Addremoteobject08190] \( #17 \)[に移動][GithubMicrosoftedgeWebviewfeedbackIssue17]します。  
*   イベントが追加 `AcceleratorKeyPressed` されました。  詳細については、\( [add_AcceleratorKeyPressed][ReferenceWin32Iwebview2webview4AddAcceleratorkeypressed08190] \)[に#57][GithubMicrosoftedgeWebviewfeedbackIssue57]してください。  
*   無効 `Context Menus` です。  詳細については、\( [put_AreDefaultContextMenusEnabled][ReferenceWin32Iwebview2settings2PutAredefaultcontextmenusenabled08190] \)[に][GithubMicrosoftedgeWebviewfeedbackIssue57]#57してください。  
*   更新されました `DPI Awareness` 。  現在、WebView の DPI 対応はホスト アプリケーションの DPI 対応と同じです。  
    
    > [!NOTE]
    > 別のハイブリッド アプリケーションが元の WebView とは異なる DPI 対応で起動された場合、ユーザー データ ディレクトリが同じ \( #1 \) の場合、新しい WebView[は起動][GithubMicrosoftedgeWebviewfeedbackIssue1]されません。  
    
*   WebView2 が、WebView 内でホストされている Web コンテンツによって要求される通知アクセス許可要求 `Notification Change Behavior` を自動的に拒否するように更新されました。  

## 0.8.270  

[NuGet パッケージ][NuGetGallery0.8.270] \|Microsoft Edge バージョン 78.0.270.0 以上。  
...
*   指定しない場合 `CreateWebView2EnvironmentWithDetails` に選択するロジックを使用して関数 `userDataFolder` を更新しました。  関数の詳細については `CreateWebView2EnvironmentWithDetails` [、CreateWebView2EnvironmentWithDetails に移動します][ReferenceWin32WebView2IdlCreatewebview2environmentwithdetails08190]。  以前に既定の場所を使用した場合、新しい SDK に切り替える場合、既定はリセット \(ホスト コード ディレクトリの新しい場所に設定\) され、状態もリセット `userDataFolder` `userDataFolder` されます。  
    指定されたディレクトリに書き込むアクセス許可がホスト プロセスに付与されていない場合、関数 `CreateWebView2EnvironmentWithDetails` は失敗する可能性があります。  古いユーザー データ ディレクトリから新しいディレクトリにデータをコピーできます。  

## 0.8.230  

[NuGet パッケージ][NuGetGallery0.8.230] \|Microsoft Edge バージョン 77.0.230.0 以上。  
...
[NuGet パッケージ][NuGetGallery0.8.190] \|Microsoft Edge バージョン 77.0.190.0 以上。  

*   ユーザーが `get_AreDevToolsEnabled` / `put_AreDevToolsEnabled` DevTools \( \#16 \) を開くことができるか制御[するために追加][GithubMicrosoftedgeWebviewfeedbackIssue16]されます。  
...
*   スクリプトでの要求 `window.open()` を有効にします。  

## 0.8.149  

[NuGet パッケージ][NuGetGallery0.8.149] \|Microsoft Edge バージョン 76.0.149.0 以上。  
...
[ConceptsDistributionEvergreenMode]: ./concepts/distribution.md#evergreen-distribution-mode "常用配布モード - WebView2 を使用したアプリケーションの配布 |Microsoft Docs"  
[Webview2ConceptsDistributionFixedVersionMode]: ./concepts/distribution.md#fixed-version-distribution-mode "固定バージョン配布モード - WebView2 を使用したアプリケーションの配布 |Microsoft Docs"  
...
[ConceptsVersioningExperimentalApis]: ./concepts/versioning.md#experimental-apis "試験的な API - ブラウザーのバージョンと WebView2 について |Microsoft Docs"  
[GettingstartedWinforms]: ./gettingstarted/winforms.md "Windows フォーム アプリでの WebView2 の概要 |Microsoft Docs"  
[GettingstartedWpf]: ./gettingstarted/wpf.md "WPF での WebView2 の概要 |Microsoft Docs"  
[HowtoDebug]: ./howto/debug.md "WebView2 コントロールを使用して開発するときにデバッグする方法 |Microsoft Docs"  
...
[DotnetApiSystemEventargs]: /dotnet/api/system.eventargs "EventArgs クラス (システム) |Microsoft Docs"  
[DotnetStandardAssemblyStrongNamed]: /dotnet/standard/assembly/strong-named "名前付きアセンブリ |Microsoft Docs"  
...
[GithubMicrosoftedgeWebview2samplesPr17]: https://github.com/MicrosoftEdge/WebView2Samples/pull/17 "最新の WebView2 SDK 0.9.430 を使用するプロジェクトの移動 - MicrosoftEdge/WebView2Samples |GitHub"  
...
[MicrosoftDevblogDotnetAnnouncingGeneralAvailabilityForMicrosoftEdgeWebview2ForNetFixedDistributionMethod]: https://devblogs.microsoft.com/dotnet/announcing-general-availability-for-microsoft-edge-webview2-for-net-and-fixed-distribution-method ".NET および固定配布方法に関する Microsoft Edge WebView2 の一般提供について発表 | .NET ブログ"  
...
[WindowsBlogsMsedgedevEdgeWebview2GeneralAvailability]: https://blogs.windows.com/msedgedev/edge-webview2-general-availability "Microsoft Edge WebView2 の一般提供について |Microsoft Edge ブログ"  
...
[DOMContentLoadedAPI]: /microsoft-edge/webview2/reference/win32/icorewebview2_2?view=webview2-1.0.721-prerelease#add_domcontentloaded&preserve-view=true "add_DOMContentLoaded - インターフェイス ICoreWebView2_2 |Microsoft Docs"
