...
CoreWebView2HttpHeadersCollectionIterator | HTTP ヘッダーのコレクションの Iterator。
CoreWebView2HttpRequestHeaders | HTTP 要求ヘッダー。
CoreWebView2HttpResponseHeaders | HTTP 応答ヘッダー。
...
CoreWebView2WebResourceRequest | WebResourceRequested イベントと共に使用される HTTP 要求。
CoreWebView2WebResourceRequestedEventArgs | WebResourceRequested イベントのイベント引数。
CoreWebView2WebResourceResponse | WebResourceRequested イベントと共に使用される HTTP 応答。
