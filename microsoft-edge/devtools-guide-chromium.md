...
フロントエンド JavaScript コードからログを表示したい場合、またはスクリプトをすばやく実行する場合は、 `Ctrl` + `Shift` + `J` Windows \ (macOS) を押して、 `Command` + `Option` + `J` devtools でコンソールパネルを起動します。  
...
*   ログメッセージを表示してフィルター処理し、JavaScript オブジェクトと DOM ノードを検査し、選択したウィンドウまたはフレームのコンテキストで JavaScript を実行する**本体**  
*   コードのオープンとライブ編集、ブレークポイントの設定、コードのステップ実行、web サイトの1行の JavaScript の状態の表示を行うための**ソース**パネル  
*   ネットワークとブラウザーのキャッシュから要求と応答を監視および検査するための**ネットワーク**パネル   
*   サイトで必要な時間とシステムリソースをプロファイルするための**パフォーマンス**パネル  
*   メモリリソースの使用を測定し、コードランタイムのさまざまな状態でヒープスナップショットを比較するための**メモリ**パネル  
...
| 操作 | Windows | macOS |  
|:--- |:--- | :--- |  
| DevTools の表示/非表示を切り替える \ (最後に表示したパネルに表示されます) | `F12` /`Ctrl`+`Shift`+`I` | `Command`+`Option`+`I` |  
...
| 編集不可の HTML ソースコードを新しいタブに表示する | `Ctrl`+`U` | なし |  
