...
簡単な Web アプリが作成されたので、PWA として拡張するには、PWA の 3 つの要件を追加します。<!--[3 requirements for PWAs][PwaEdgehtmlIndexRequirements]-->: [HTTPS](#step-1---use-https) [、Web アプリ マニフェスト、](#step-2---create-a-web-app-manifest)および [サービス ワーカー](#step-3---add-a-service-worker)。  
